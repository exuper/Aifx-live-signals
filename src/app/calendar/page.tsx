...
export default function CalendarPage() {
  ...
  const eventDays = Object.keys(events).sort();
  ...
  return (
    <div className="space-y-8">
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          {eventDays.length === 0 && !isLoading && (
             <div className="text-center text-muted-foreground py-10 border-2 border-dashed border-muted rounded-lg">
                <p className='font-semibold'>No Events Found</p>
                <p className='text-sm'>Calendar events can be added by an admin in the dashboard.</p>
              </div>
          )}
          {eventDays.map((day) => (
            <div key={day}>
              <h2 className="text-xl font-bold font-headline text-primary mb-4">{format(new Date(day), 'EEEE, MMMM d, yyyy')}</h2>
