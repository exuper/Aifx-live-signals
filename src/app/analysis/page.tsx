  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        if (chartDataUri) {
          const result = await analyzeTechnicalChartAction({ chartDataUri });
          setAnalysis(result);
        } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="font-headline">Upload Chart Image</CardTitle>
            <CardDescription>Drag & drop or click to select a PNG or JPG file.</CardDescription>
          </CardHeader>
          ...
          {!analysis && !isLoading && (
              <Card className="flex flex-col items-center justify-center h-full text-center border-dashed">
                  <CardContent className="p-10">
                      ...
                      <p className="text-muted-foreground">Upload a chart to see the AI's insights here.</p>
                  </CardContent>
              </Card>
          )}
        </div>
